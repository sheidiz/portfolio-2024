...
export const ProjectMobile = ({project}) => {
  ...
  const {title, description, image1, image2, image3, technologies, web, repository, figma, video} = project;
  ...
  return (
    <div className="bg-tertiary/90 dark:bg-secondary/90 rounded-2xl p-4 flex flex-col items-center justify-evenly hover:bg-tertiary hover:dark:bg-secondary">
            ...
            <div className='mt-5 h-44 md:h-52 w-64 md:w-full lg:w-4/5 mx-auto flex justify-evenly'>
              <img src={image1} alt={title + "1"} className='h-36 md:h-44 shadow-xl duration-150 hover:scale-110' />
              <img src={image2} alt={title + "2"} className='h-36 md:h-44 shadow-xl duration-150 hover:scale-110' />
              <img src={image3} alt={title + "3"} className='h-36 md:h-44 shadow-xl duration-150 hover:scale-110' />
            </div>
